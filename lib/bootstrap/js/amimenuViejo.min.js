...
init();return{show:manualShow,close:close,reset:reset};})(this);(function($){if(!$||!($.toJSON||Object.toJSON||window.JSON)){throw new Error("jQuery, MooTools or Prototype needs to be loaded before jStorage!");}
var
...
+data[i].id
+')" class="btn btn-primary">Go</a>';if(data[i].tel){itemshtml+='<a a href="tel:'+data[i].tel
+'" class="btn">Call</a>';}
itemshtml+='</p></div></div></li>';$('.thumbnails').append(itemshtml);}}
function liveRadiusSearch(lat,lng){users.clearLayers();var radius=$('#radiustosend').html();$.jStorage.set("lastradius",radius);$.jStorage.set("lastlat",lat);$.jStorage.set("lastlng",lng);$.getJSON("getbusinessradius.php?lat="+lat+"&lng="+lng
+"&radius="+radius,function(data){for(var i=0;i<data.length;i++){var location=new L.LatLng(data[i].glat,data[i].glng);var name=data[i].nom;if(data[i].nom!=null){var title="<div style='font-size: 18px; color: #0078A8;'><a href='#' onClick='getRestaurant("
+data[i].id
+");'>"
+data[i].nom
+"</a><br/><a href='tel:"
...
+"</a><article>"
+data[i].intro
+"</article></div>";if(data[i].ville!=null){var city="<div style='font-size: 14px;'>"
+data[i].ville+"</div>";}else{var city="";}
var marker=new L.Marker(location);marker.bindPopup("<div style='text-align: center; margin-left: auto; margin-right: auto;'>"
+title
+city
+"<span><a rel='localize[btntoque]' class='btn btn-success btn-small' onClick='opentoque("
+data[i].tel
+");'>Toque</a></span></div>",{maxWidth:'300',minWidth:'150'});users.addLayer(marker);}}
setListItems(data);}).complete(function(){map.fitBounds(users.getBounds());if(firstLoad==true){map.fitBounds(users.getBounds());firstLoad=true;};});}
function liveSearch(param,query){users.clearLayers();if(param=="city"){$.jStorage.set("lastcity",query);}
$.getJSON("get_restaurants.php?"+param+"="+query,function(data){for(var i=0;i<data.length;i++){var location=new L.LatLng(data[i].glat,data[i].glng);var name=data[i].nom;if(data[i].nom!=null){var title="<div style='font-size: 18px; color: #0078A8;'><a href='#' onClick='getRestaurant("
+data[i].id
+")'>"
+data[i].nom
+"</a><br/><a href='tel:"
...
+");'>Toque</a></span></div>",{maxWidth:'300',minWidth:'150'});users.addLayer(marker);}}
setListItems(data);}).complete(function(){map.fitBounds(users.getBounds());if(firstLoad==true){map.fitBounds(users.getBounds());firstLoad=true;};});}
function getRestaurants(){$.getJSON("get_restaurants.php",function(data){for(var i=0;i<data.length;i++){var location=new L.LatLng(data[i].glat,data[i].glng);var name=data[i].nom;if(data[i].nom!=null){var title="<div style='font-size: 18px; color: #0078A8;'><a href='#' onClick='getRestaurant("
+data[i].id
+")'>"
+data[i].nom
+"</a><br/><a href='tel:"
...
+"'>"
+data[i].tel
+"</a>";if(data[i].intro!=null&&data[i].intro!='null'){title+="</div>";}else{title+="</div>";}
if(data[i].ville!=null){var city="<div style='font-size: 14px;'>"
+data[i].ville+"</div>";}else{var city="";}
var marker=new L.Marker(location);marker.bindPopup("<div style='text-align: center; margin-left: auto; margin-right: auto;'>"
+title
+city
...
+data[i].tel
+");'>Toque</a></span></div>",{maxWidth:'300',minWidth:'150'});users.addLayer(marker);}}
setListItems(data);}).complete(function(){if(firstLoad==true){map.fitBounds(users.getBounds());firstLoad=false;};});}
function opentoque(tel){$('#dartoqueModal').modal('show');$('#teltosend').html(tel);}
var file;function prepareUpload(){document.getElementById('fileSize').innerHTML='';file=document.getElementById('fileToUpload').value;if(file.lastIndexOf('\\')>=0)
file=file.substr(file.lastIndexOf('\\')+1);document.getElementById('fileName').innerHTML=file;var curFolder=window.location.href;if(curFolder[curFolder.length-1]!='/')
curFolder=curFolder.substring(0,curFolder.lastIndexOf('/')+1);document.getElementById('target').innerHTML=curFolder;document.getElementById('inputform').action=curFolder;}
function insertRestaurant(){var name=$("#name").val();var phone=$("#phone").val();var intro=$("#intro").val();var email=$("#email").val();var website=$("#website").val();var city=$("#city").val();var lat=$("#lat").val();var lng=$("#lng").val();var introlunes=$("#mondaymenu").val();var intromartes=$("#tuesdaymenu").val();if(name.length==0){$('#checkform').val("Name is required!");$('#checkerror').show();return false;}
if(email.length==0){alert("Email is required!");return false;}
...
+'&intromartes='+intromartes+'&fileToUpload='+file;$.ajax({type:"POST",url:"insert_restaurant.php",data:dataString,success:function(){cancelRegistration();users.clearLayers();getCountries();getRestaurants();$('#insertSuccessModal').modal('show');},error:function(){cancelRegistration();users.clearLayers();$('#insertFailModal').modal('show');},complete:function(){cancelRegistration();users.clearLayers();getCountries();getRestaurants();$('#insertSuccessModal').modal('show');}});return false;}
function getRestaurantToEdit(id){$.jStorage.set("lastrestedit",id);$.getJSON("get_restaurant.php?restaurantid="+id,function(data){for(var i=0;i<data.length;i++){var idrest=data[i].id;$('#idrestedit').val(idrest);var image=data[i].img;if(image!=null){$('#imageedit').attr('src','uploads/50_'+image);$('#completeimagesource').attr('src','uploads/500_'+image);}else{$('#imageedit').attr('src','uploads/pordefecto.png');$('#completeimagesource').attr('src','uploads/pordefecto.png');}
var name=data[i].nom;$('#nameedit').val(name);$('#nombreimagenrestedit').text(name);var pos=data[i].codepostal;$('#postcodeedit').val(pos);var est=data[i].departement;$('#stateedit').val(est);var cou=data[i].pay;$('#countryedit').val(cou);var add=data[i].adresse;$('#addresseedit').val(add);var city=data[i].ville;$('#cityedit').val(city);var lat=data[i].glat;$('#latedit').val(lat);var lng=data[i].glng;$('#lngedit').val(lng);var phone=data[i].tel;$('#phoneedit').val(phone);var mobile=data[i].tel2;$('#mobileedit').val(mobile);var addre=data[i].adresse;$('#addressedit').val(addre);var mail=data[i].email;$('#emailedit').val(mail);$('#emailedit').attr('href','mailto:'+mail);var web=data[i].web;$('#websiteedit').val(web);$('#websiteedit').attr('href',web);var intro=data[i].intro;if(intro!=null){$('#introedit').val(intro);}
var lu=data[i].introlunes;if(lu!=null){$('#introlunesedit').val(lu);}
var ma=data[i].intromartes;if(ma!=null){$('#intromartesedit').val(ma);}
var mi=data[i].intromiercoles;if(mi!=null){$('#intromiercolesedit').val(mi);}
var ju=data[i].introjueves;if(ju!=null){$('#introjuevesedit').val(ju);}
var vi=data[i].introviernes;if(vi!=null){$('#introviernesedit').val(vi);}
var sa=data[i].introsabado;if(sa!=null){$('#introsabadoedit').val(sa);}
var domi=data[i].introdomingo;if(domi!=null){$('#introdomingoedit').val(domi);}}}).complete(function(){});}
function getRestaurant(id){$.getJSON("get_restaurant.php?restaurantid="+id,function(data){for(var i=0;i<data.length;i++){var image=data[i].img;if(image!=null){$('#detailsimage').attr('src','uploads/50_'+image);$('#completeimagesource').attr('src','uploads/500_'+image);}else{$('#detailsimage').attr('src','uploads/pordefecto.png');$('#completeimagesource').attr('src','uploads/pordefecto.png');}
var name=data[i].nom;$('#detailsname').text(name);$('#nombreimagenrest').text(name);var phone=data[i].tel;$('#detailsphone').text(phone);var mobile=data[i].tel2;$('#detailsmobile').text(mobile);var addre=data[i].adresse;$('#detailsaddress').text(addre);var mail=data[i].email;$('#detailsmail').text(mail);$('#detailsmail').attr('href','mailto:'+mail);var web=data[i].web;$('#detailsweb').html(web);$('#detailsweb').attr('href',web);var intro=data[i].intro;if(intro!=null){$('#detailsinfo').html(intro);}
var lu=data[i].introlunes;if(lu!=null){$('#introlunesdetail').html(lu);}
var ma=data[i].intromartes;if(ma!=null){$('#intromartesdetail').html(ma);}
var mi=data[i].intromiercoles;if(mi!=null){$('#intromiercolesdetail').html(mi);}
var ju=data[i].introjueves;if(ju!=null){$('#introjuevesdetail').html(ju);}
var vi=data[i].introviernes;if(vi!=null){$('#introviernesdetail').html(vi);}
var sa=data[i].introsabado;if(sa!=null){$('#introsabadodetail').html(sa);}
var domi=data[i].introdomingo;if(domi!=null){$('#introdomingodetail').html(domi);}
$('#shareme').attr('data-url','http://amimenu.mowaps.com/details?'+id);$('#shareme').sharrre({share:{twitter:true,facebook:true,pinterest:true},template:'<div class="box"><div id="sharename" class="left">Share</div><div class="middle"><a href="#" class="facebook">f</a><a href="#" class="twitter">t</a><a href="#" class="pinterest">p</a><a href="#" class="googleplus">+1</a></div><div class="right">{total}</div></div>',enableHover:false,enableTracking:true,render:function(api,options){$(api.element).on('click','.twitter',function(){api.openPopup('twitter');});$(api.element).on('click','.facebook',function(){api.openPopup('facebook');});$(api.element).on('click','.googleplus',function(){api.openPopup('googlePlus');});$(api.element).on('click','.pinterest',function(){api.openPopup('pinterest');});}});}}).complete(function(){$('#restaurantDetails').modal('show');});}
function sendToque(){var name=$("#name_toque").val();var email=$("#email_toque").val();var phone=$("#phone_toque").val();var phonerest=$("#teltosend").html();if(name.length==0){alert("Name is required!");return false;}
if(email.length==0){alert("Email is required!");return false;}
if(phone.length==0){alert("Phone is required!");return false;}
var dataString='email='+email+'&usuario='+phone+'&name='+name
+'&phonerest='+phonerest+'&callback=0';$.ajax({url:"freesms.php",data:dataString,success:function(data){if(data==0){$("#oksms").show();}else if(data==1){$("#failsms").show();}else{$("#tellsms").show();}}});return false;}
function removeRestaurant(){var email=$("#email_remove").val();var token=$("#token_remove").val();if(email.length==0){alert("Email is required!");return false;}
if(token.length==0){alert("Token is required!");return false;}
var dataString='email='+email+'&token='+token;$.ajax({type:"POST",url:"remove_restaurant.php",data:dataString,success:function(data){if(data>0){$('#removemeModal').modal('hide');users.clearLayers();getRestaurants();$('#removeSuccessModal').modal('show');}else{alert("Incorrect email or token. Please try again.");}}});return false;}
function editRestaurant(){var email=$("#email_edit").val();var token=$("#token_edit").val();if(email.length==0){alert("Email is required!");return false;}
if(token.length==0){alert("Token is required!");return false;}
var dataString='email='+email+'&token='+token;$.getJSON("edit_restaurant.php?"+dataString,function(data){for(var i=0;i<data.length;i++){console.log(data[i].id);if(data.length>0){getRestaurantToEdit(data[i].id);$('#editmeModal').modal('hide');$('#editSuccessModal').modal('show');}else{alert("Incorrect email or token. Please try again.");}}});return false;}
function onMapClick(e){var lat=e.latlng.lat.toFixed(6);$('#lat').val(lat);$('#lng').val(e.latlng.lng.toFixed(6));$('#registerModal').modal('show');getLocate(lat,e.latlng.lng.toFixed(6));}
if(jQuery)(function($){$.extend($.fn,{dropdown:function(method,data){switch(method){case'hide':hideDropdowns();return $(this);case'attach':return $(this).attr('data-dropdown',data);case'detach':hideDropdowns();return $(this).removeAttr('data-dropdown');case'disable':return $(this).addClass('dropdown-disabled');case'enable':hideDropdowns();return $(this).removeClass('dropdown-disabled');}}});function showMenu(event){var trigger=$(this),dropdown=$($(this).attr('data-dropdown')),isOpen=trigger.hasClass('dropdown-open');event.preventDefault();event.stopPropagation();hideDropdowns();if(isOpen||trigger.hasClass('dropdown-disabled'))return;dropdown.css({left:dropdown.hasClass('anchor-right')?trigger.offset().left-(dropdown.outerWidth()-trigger.outerWidth()):trigger.offset().left,top:trigger.offset().top+trigger.outerHeight()}).show();trigger.addClass('dropdown-open');};function hideDropdowns(event){var targetGroup=event?$(event.target).parents().andSelf():null;if(targetGroup&&targetGroup.is('.dropdown-menu')&&!targetGroup.is('A'))return;$('BODY').find('.dropdown-menu').hide().end().find('[data-dropdown]').removeClass('dropdown-open');};$(function(){$('BODY').on('click.dropdown','[data-dropdown]',showMenu);$('HTML').on('click.dropdown',hideDropdowns)});})(jQuery);